...
// 它會自動過濾空白與註解，並辨識關鍵字（func, if, return, while）或識別碼（ID）。
// =========================================================
typedef enum {
...
void next_token() {
    ...
    // 辨識識別碼 (ID) 與 關鍵字 (Keyword)
        ...
        else if (strcmp(cur_token.text, "while") == 0) cur_token.type = TK_WHILE; // 新增 while 支援
...
// 處理陳述句 (Assignment, If, While, Return)
void statement() {
    ...
    // 新增 while 迴圈的語法解析
        ...
        int loop_start = quad_count; // 記錄條件判斷開始的位址
        next_token(); next_token(); char cond[32]; expression(cond);
        next_token(); next_token(); 
        
        int exit_jmp_idx = quad_count; // 記下條件不成立時的跳轉指令位置
        emit("JMP_F", cond, "-", "?"); 
        
        while (cur_token.type != TK_RBRACE) statement();
        next_token(); 
        
        char loop_start_str[32];
        sprintf(loop_start_str, "%d", loop_start);
        emit("JMP", "-", "-", loop_start_str); // 迴圈結尾，無條件跳回開頭判斷條件
        
        sprintf(quads[exit_jmp_idx].result, "%d", quad_count); // 回填 JMP_F 離開迴圈的地址
    }
    else if (cur_token.type == TK_ID) {
        char name[32]; strcpy(name, cur_token.text); next_token();
